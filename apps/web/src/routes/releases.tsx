...
function ReleaseCard({
  ...
  return (
    <AccordionItem value={release.tag_name} className="border-0">
      <Card
      ...
      >
        <AccordionTrigger className="w-full p-0 text-left hover:no-underline [&>svg]:hidden">
          <CardHeader className="w-full cursor-pointer transition-colors hover:bg-white/5">
            ...
            {release.body && (
              <CardDescription className="line-clamp-2">
                {release.body.replaceAll(/[#*`]/g, '').slice(0, 200)}...
              </CardDescription>
            )}
          </CardHeader>
        </AccordionTrigger>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-slate-400 text-xs">
                          {formatBytes(asset.size)} • {asset.download_count}{' '}
...
function ReleasesList({ releases }: Readonly<{ releases: GitHubRelease[] }>) {
  ...
  // Default to first release (latest) being open
  const defaultValue = releases.length > 0 ? releases[0].tag_name : undefined;
