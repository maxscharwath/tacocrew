...
export class TacosApiClient {
  ...
  constructor(
    private baseUrl: string = config.backend.baseUrl,
    private csrfPath: string = 'index.php?content=livraison'
  ) {
  ...
  private buildCsrfUrl(): string {
    const normalizedPath = this.csrfPath.startsWith('/') ? this.csrfPath.slice(1) : this.csrfPath;
    return `${this.baseUrl}/${normalizedPath}`;
  }

  private async requestCsrfPage(cookieHeader?: string): Promise<AxiosResponse<string>> {
    const candidateUrls = [this.buildCsrfUrl(), `${this.baseUrl}/`];
    ...
    for (const url of candidateUrls) {
      try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 404) {
          lastError = error;
          logger.warn('CSRF page returned 404, trying fallback', { url });
          continue;
        }
        lastError = error;
        break;
    ...
    throw lastError instanceof Error
      ? lastError
      : new CsrfError(`Failed to fetch CSRF token page at ${this.baseUrl}`);
  }
