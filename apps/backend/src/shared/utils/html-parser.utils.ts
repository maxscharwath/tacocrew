...
function findLabeledParagraph($scope: Cheerio<any>, labels: string[]): Cheerio<any> {
  let $p = $scope
  ...
  if (el?.tagName && el.tagName !== 'p') {
    $p = $p.closest('p') as Cheerio<any>;
  }
  return $p;
}

function extractValueAfterColonFromParagraph($p: Cheerio<any>): string | null {
...
function parseListFromLabeledParagraph($scope: Cheerio<any>, labels: string[]): string[] {
