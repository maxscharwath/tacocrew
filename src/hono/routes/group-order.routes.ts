...
import { jsonContent } from '@/hono/routes/shared.schemas';
...
app.openapi(
  ...
  async (c) => {
    ...
    return c.json(GroupOrderMapper.toResponseDto(groupOrder), 200);
  }
);
...
app.openapi(
  ...
  async (c) => {
    ...
    return c.json(GroupOrderMapper.toResponseDtoWithUserOrders(groupOrder, userOrders), 200);
  }
);
...
app.openapi(
  ...
  async (c) => {
    ...
    return c.json(UserOrderMapper.toResponseDto(userOrder), 200);
  }
);
...
app.openapi(
  ...
  async (c) => {
    ...
    const userId = requireUserId(c);
    const body = c.req.valid('json');
    // Body is already validated with branded ID types by the schema
...
app.openapi(
  ...
  async (c) => {
    ...
    return c.json(
      userOrders.map((uo) => UserOrderMapper.toResponseDto(uo)),
      200
    );
  }
);
